...
def normalize_data_series(series_list):
    ...
    try:
        ...
        clean_data = series_list.copy()

        clean_data["year"] = clean_year(series_list.get("year"))
        clean_data["rating"] = clean_rating(series_list.get("rating"))
        clean_data["title"] = series_list.get("title", "").title()
        normalized_series.append(clean_data)
        return {
            "status": "success",
